(ns sysanalysis.ledger
  "Append-only audit ledger for the ISCO-08 2511 community systems-analysis
  ...
  audit is trying to check. For a record of which designs were accepted as
  totally covering their requirements, that is the difference between an audit
  that can show which designs were approved and one that can only show which
  designs someone chose to leave in the file.
  ...
  production `:approve-cutover` committed through human sign-off and a routine
  `:propose-design` committed automatically both produced `{:disposition
  :commit :record ...}` with no field distinguishing them. `commit-entry`
  therefore takes the approval provenance as part of the record, not as an
  afterthought.")
...
(defn summary
  "One line per entry, for operators and for `sysanalysis.sim` output."
  [ledger]
  (apply str
         (interpose "\n"
                    (for [e ledger]
