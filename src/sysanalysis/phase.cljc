(ns sysanalysis.phase
  "The verdict -> phase mapping for the ISCO-08 2511 community
  systems-analysis actor, and what each phase is allowed to do.
  ...
  design record is written, held, or sent to a human had no test of its own and
  ...
  request most likely to be waved through: `:approve-cutover` is an escalating
  operation *and* a design-binding one, so it is both the only op that reaches
  a human by construction and an op that can carry an uncovered requirement, a
  foreign component or an invented requirement.

  Note precisely what this ordering does and does not currently protect.
  `sysanalysis.governor` already computes `:escalate?` as `(and (not hard?)
  ...
  function and NOT by `sysanalysis.sim` — reversing the two clauses leaves the
  whole scenario table green. That is stated rather than left for a reader to
...
(defn refusal?
  ...
  without a human, not an approval-in-waiting. `sysanalysis.sim` counts these;
...
(defn approved-commit?
  ...
  pre-change tree, it could not: a production cutover approved by a human and a
  routine design committed automatically left two `{:disposition :commit ...}`
  entries with no field telling them apart. For a cutover being switched on in
  production, that distinction is the entire reason the interrupt exists."
  [disposition]
  (= :request-approval disposition))

(defn escalating-op?
  "True for operations that reach `:request-approval` by declaration rather
  than by confidence. Re-exported from `sysanalysis.operation` so a reader of
  the routing rule can see which ops always take the human path without
  following a second namespace."
  [o]
  (op/escalates? o))
