(ns sysanalysis.actor
  "SystemsAnalysisActor — the ISCO-08 2511 community sysanalysis actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one sysanalysis operation request
  ...
  The unconditional invariant: the SystemsAnalysisAdvisor can never
  directly commit a record the SystemsAnalysisGovernor refuses —
...
(defn build-graph
  "Build a compiled SystemsAnalysisActor graph. `store` implements
  `sysanalysis.store/Store`. `advisor` implements
  `sysanalysis.advisor/Advisor` (defaults to `mock-advisor`).
